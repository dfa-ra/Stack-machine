...
class CompilerText:
        ...
        for line in lines:
            ...
            for token in tokens:
                if token == "LOOP":
                    ...
                    self.loop_flag = 2
                    self.loop_sample[
                        len(self.loop_sample) - 1
                    ].compare_token = self.compare_token
                    self.scope.add_scope([])
                elif token == "REPEAT":
                    self.loop_flag = 3
                    ...
                    self.compiler.pc += len(
                        self.loop_sample[-1].steps_to_check_data
                    ) + len(self.loop_sample[-1].increase_counter)
    ...
    def compile_text(self, token: str, address_space: int) -> None:
        if (
            len(self.compiler.symbols) > 0
            and address_space < len(self.compiler.symbols)
            and token in self.compiler.symbols[address_space]
        ):
            ...
            if len(token) > 1:
                if token[1] == "+" and len(token) == 2:
                    ...
                    if (
                        len(self.compiler.symbols) == 0
                        or name not in self.compiler.symbols[address_space].keys()
                    ):
                        if len(self.compiler.symbols) == 0:
                            self.compiler.symbols.append({})
                        self.compiler.symbols[address_space][name] = Symbol(
                            self.intermediate_var, "var", 1, [0]
                        )
                        if self.loop_flag == 1:
                            self.loop_sample[
                                len(self.loop_sample) - 1
                            ].counter_address = self.intermediate_var
                        self.intermediate_var += 4
                        self.emit(
                            "sw_to_imm_addr",
                            self.compiler.symbols[address_space][name].address,
                        )
