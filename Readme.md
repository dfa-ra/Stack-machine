...
- Forth код компилируется в язык мнемоник который кладётся в директорию build     vn
...
### Память данных
 - Представляет собой последовательность байт адресуемых по машинному слову (32 бита в little-endian)
...
Бинарное представление инструкций (всё представлено в little-endian)
...
7. **[ssm](./tests/golden/ssm)** и **[vsm](./tests/golden/vsm)** tests - эти 2 теста получают на вход 2 массива и должны сложить элементы попарно и вывести результирующий массив однако тест `ssm` делает это с помощью скалярных операций, а тест `vsm` с помощью векторных.
   ...
   - Векторно: 4 такта (загрузка 2 векторов по 2 такта каждый) + 1 такт (сложение 2 векторов) + 2 такта (запись результирующего вектора) = 7 тактов

    Итого выйгрыш чуть больше чем в 2 раза, а если быть точнее в 2.3 раза. 


---

## Как запустить? 
Я надеюсь что до сюда никто не долистал так как так проблемы с зависимостями, но если что сборки есть, но не со всеми либами... будем считать что это квест для тех кто решит попользоваться 
### Установка из бинарного релиза
  1. Откройте последнюю основную сборку в [Actions](https://github.com/dfa-ra/Stack-machine/actions)
  2. Загрузите zip архив для вашей платформы
  3. Разархивируйте zip архив в папку в которой будете работать
  4. Запустите код 

(PS: обладатели windows можете помолиться перед запуском так как иначе оно не запуститься) 
### Запуск
Запуск осуществляется с помощью бинарного файла stack-machine.

Пример запуска:
`./stack-machine -c arrays.yaml -f arrays.forth`

Флаги: 
- `-c` - флаг показывающий, что следующий файл это файл конфигурации
- `-f` - флаг показывающий, что следующий файл это файл программы на forth
